...
export default function useOrderRealtime({
  ...
  const initializeConnection = useCallback(async () => {
    ...
    try {
        ...
        if (data.order && data.order.order_id == orderId) {
          ...
          setTimeout(() => {
            const statusElements = document.querySelectorAll(
              ".order-status-pending, .order-status-confirmed, .order-status-shipping, .order-status-shipped, .order-status-delivered"
            );
      ...
      const returnChannel = pusher.subscribe(`return-request.${returnId}`);
      channelsRef.current.returnChannel = returnChannel;

      returnChannel.bind("pusher:subscription_succeeded", () => {});

      returnChannel.bind("return-request-updated", (data) => {
        setLastUpdate(new Date().toISOString());
